/-
  ...
  - T1: cpOp is associative (Lemma 2)
...
/-! ## cpOp identity lemmas -/

@[simp] theorem cpOp_id_right (a : CarryPair) : cpOp a (false, true) = a := by
  obtain ⟨g, p⟩ := a; simp [cpOp]

@[simp] theorem cpOp_id_left (a : CarryPair) : cpOp (false, true) a = a := by
  obtain ⟨g, p⟩ := a; simp [cpOp]

/-! ## T1: Associativity of cpOp (Brent-Kung Lemma 2) -/

theorem cpOp_assoc (a b c : CarryPair) :
    cpOp (cpOp a b) c = cpOp a (cpOp b c) := by
...
theorem carry_eq_blockGenerate (i : Nat) (x y : BitVec w) :
    BitVec.carry i x y false = (blockGP i x y).1 := by
  induction i with
  | zero => simp [blockGP]
  | succ n ih =>
    simp only [BitVec.carry_succ, atLeastTwo_eq_gp, blockGP_succ, cpOp_fst,
...
def blockGP_range (lo hi : Nat) (x y : BitVec w) : CarryPair :=
  match hi with
  | 0 => (false, true)
  | n + 1 =>
    if n < lo then (false, true)
    else cpOp (generate n x y, propagate n x y) (blockGP_range lo n x y)
...
theorem blockGP_split (i j : Nat) (hj : j ≤ i) (x y : BitVec w) :
    blockGP i x y = cpOp (blockGP_range j i x y) (blockGP j x y) := by
